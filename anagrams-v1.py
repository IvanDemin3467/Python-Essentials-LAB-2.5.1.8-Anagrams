...
def is_anagram(text1, text2):
    ...
    text2 = text2.lower()                # same
    text1 = text1.replace(" ", "")       # spaces are allowed
    text2 = text2.replace(" ", "")       # same
    if len(text1) != len(text2):         # only strings of same length allowed
        return False                     # if the lengths differ -> terminate function
    
    is_anagr = True                      # let's assume, that we have anagrams
    for char in text1:                   # iterate through chars in first text
        if text2.find(char) != -1:       # find current char in second text
            text2.replace(char, "", 1)   # if found -> delete that entry from second text
        else:                            # if not found 
            is_anagr = False             # -> these are not anagrams
            break                        # and terminate loop
    return is_anagr                      # no extra checks needed. Job is done
